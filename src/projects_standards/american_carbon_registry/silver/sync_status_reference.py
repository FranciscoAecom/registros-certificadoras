...
REFERENCE_NAME = "American Carbon Registry"
NON_STATUS_REGULATORY_MARKERS = {"Not ARB or Ecology Eligible"}


# Coleta os status observados no projeto aplicando a regra especifica da ACR.
def collect_status_rows(project: dict[str, Any]) -> list[tuple[str, str, str | None]]:
    rows = collect_default_status_rows(project)
    return [row for row in rows if not (row[0] == "regulatory" and row[1] in NON_STATUS_REGULATORY_MARKERS)]
