...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Baixa os detalhes brutos dos projetos da Social Carbon."
    )
...
def build_paths(snapshot_date: str) -> tuple[Path, Path, Path]:
    ...
    list_path = root / "data" / "project_standards" / "01_bronze" / "social_carbon" / snapshot_date / "list" / "projects.json"
    projects_dir = root / "data" / "project_standards" / "01_bronze" / "social_carbon" / snapshot_date / "projects"
    errors_path = (
...
def resolve_project_public_id(project: dict[str, Any], index: int) -> str:
    return sanitize_filename(str(project.get("Project ID") or f"social_carbon_project_{index}"))


# Monta o slug publico do projeto a partir dos identificadores disponiveis.
def build_project_slug(project_public_id: str, project_internal_id: str) -> str:
    return f"{project_public_id.lower()}-{project_internal_id}"
...
def fetch_project_detail(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Resposta inesperada do detalhe da Social Carbon: era esperado um objeto.")
    response_payload = payload.get("response")
    if not isinstance(response_payload, dict):
        raise RuntimeError(
            "Resposta inesperada do detalhe da Social Carbon: chave 'response' ausente ou invalida."
        )
...
def process_projects(
    *,
    ...
    for index, list_record in enumerate(target_records, start=1):
        ...
        project_internal_id = str(list_record.get("_id") or "").strip()
        if not project_internal_id:
            ...
            append_failure_entry(
                ...
                entry={
                    ...
                    "error_message": "Registro de lista sem _id.",
        ...
        try:
            detail_payload = fetch_project_detail(
                project_internal_id=project_internal_id,
                timeout=timeout,
                retry_attempts=retry_attempts,
                retry_sleep_seconds=retry_sleep_seconds,
            ...
            payload = {
                "source": build_project_source(
                    ...
                    project_url=project_url,
                    extra_fields={
                        "detail_api_url": DETAIL_API_URL_TEMPLATE.format(
                            project_internal_id=project_internal_id
                        ),
                        "list_page_url": LIST_PAGE_URL,
                        "extraction_method": "bubble_data_api_public_obj_project_by_id",
                    },
                ),
                "list_data": list_record,
            ...
            success_count += 1
            print(f"fim download do projeto {project_public_id} (_id={project_internal_id})")
...
def main() -> int:
    ...
    print("Iniciando extracao de detalhes dos projetos da Social Carbon")
