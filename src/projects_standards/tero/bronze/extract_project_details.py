...
DETAIL_URL_TEMPLATE = "https://terocarbon.com/project/{project_public_id}/"
DETAIL_API_URL_TEMPLATE = "https://terocarbon.com/wp-json/wp/v2/project/{project_internal_id}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baixa os detalhes brutos dos projetos da TERO.")
...
def fetch_project_detail(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Resposta inesperada do detalhe da TERO: era esperado um objeto JSON.")
    return payload
...
def process_projects(
    *,
    ...
    for index, list_record in enumerate(target_records, start=1):
        ...
        project_internal_id = str(list_record.get("id") or "").strip()
        if not project_internal_id:
            ...
            append_failure_entry(
                ...
                entry={
                    ...
                    "error_message": "Registro de lista sem id.",
        ...
        project_url = DETAIL_URL_TEMPLATE.format(project_public_id=project_public_id)
        print(f"inicio download do projeto {project_public_id} ({index}/{len(target_records)})")
        try:
            detail_json = fetch_project_detail(
                project_internal_id=project_internal_id,
                timeout=timeout,
                retry_attempts=retry_attempts,
                retry_sleep_seconds=retry_sleep_seconds,
            ...
            payload = {
                "source": build_project_source(
                    ...
                    extra_fields={
                        ...
                        "extraction_method": "wordpress_rest_api_project_post_type_plus_public_html",
                    },
                ),
                "list_data": list_record,
                "detail_data": {
            ...
            success_count += 1
            print(f"fim download do projeto {project_public_id} (id={project_internal_id})")
...
def main() -> int:
    ...
    print("Iniciando extracao de detalhes dos projetos da TERO")
