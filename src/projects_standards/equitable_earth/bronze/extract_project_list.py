...
PAGE_URL = "https://registry.eq-earth.com/report/resource/PUBLIC/ERS_MEASUREMENT_STANDARD"
LIST_API_URL = "https://optimal-gateway.apx.com/reporting/api/resource/public"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Baixa a lista bruta de projetos da Equitable Earth."
    )
    ...
    parser.add_argument(
        ...
        help=f"Tamanho da pagina. Padrao: {DEFAULT_PAGE_SIZE}.",
    )
    parser.add_argument(
        "--sleep-seconds",
        type=float,
    ...
    *,
    ...
    while True:
        ...
        try:
            ...
            raise RuntimeError(
                f"Erro HTTP {exc.code} ao consultar a lista da Equitable Earth. Resposta: {details}"
            ) from exc
        except error.URLError as exc:
            raise RuntimeError(f"Falha de rede ao consultar a lista da Equitable Earth: {exc}") from exc
...
def fetch_page(
    *,
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Resposta inesperada da lista da Equitable Earth: era esperado um objeto.")
    if not isinstance(payload.get("value"), list):
        raise RuntimeError("Resposta inesperada da lista da Equitable Earth: chave 'value' ausente ou invalida.")
    return payload
...
def fetch_all_projects(
    *,
    ...
    while True:
        ...
        page_number += 1
        print(f"iniciando consulta da pagina {page_number} (skip={skip}, top={page_size})")
        ...
        batch = page_payload["value"]
        total_count_bronze = page_payload.get("@count")
        if total_count is None and isinstance(total_count_bronze, int):
            total_count = total_count_bronze
    ...
    return {
        "source": {
            ...
            "project_identifier_mapping": {
                "project_public_id": "programAssignedIdentifier",
                "project_internal_id": "resourceIdentifier",
            },
            "extraction_method": "public_reporting_api_with_apx_s_header",
        },
...
def main() -> int:
    ...
    print("Iniciando extracao da lista de projetos da Equitable Earth")
