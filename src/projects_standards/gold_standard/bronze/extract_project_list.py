...
PAGE_URL = "https://registry.goldstandard.org/projects?q=&page=1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baixa a lista bruta de projetos da Gold Standard.")
...
def build_paths(snapshot_date: str) -> tuple[Path, Path]:
    ...
    output_path = root / "data" / "project_standards" / "01_bronze" / "gold_standard" / snapshot_date / "list" / "projects.json"
...
def write_failure_log(errors_path: Path, snapshot_date: str, failures: list[dict[str, Any]]) -> None:
    ...
    payload = {
        "source": {"certificadora": "gold_standard", "snapshot_date": snapshot_date, "script": "extract_project_list.py"},
...
def fetch_page(page: int, page_size: int, timeout: float, retry_attempts: int, retry_sleep_seconds: float) -> tuple[list[dict[str, Any]], int | None]:
    query_params = parse.urlencode({"page": str(page), "size": str(page_size)})
    attempts_total = retry_attempts + 1
    attempt = 1
    while True:
        req = request.Request(url=f"{BASE_URL}?{query_params}", method="GET", headers=build_headers())
        try:
            ...
            raise RuntimeError(f"Erro HTTP {exc.code} ao consultar a Gold Standard. Resposta: {details}") from exc
        except error.URLError as exc:
            raise RuntimeError(f"Falha de rede ao consultar a Gold Standard: {exc}") from exc

        if not isinstance(payload, list):
            raise RuntimeError("Resposta inesperada da Gold Standard: era esperada uma lista de projetos.")
...
def fetch_all_projects(page_size: int, sleep_seconds: float, batch_size: int, batch_sleep_seconds: float, timeout: float, retry_attempts: int, retry_sleep_seconds: float, max_pages: int | None) -> dict[str, Any]:
    ...
    while True:
        if max_pages is not None and page > max_pages:
            break
        print(f"Iniciando consulta da pagina {page} (page={page}, size={page_size})")
        batch, batch_total_count = fetch_page(page=page, page_size=page_size, timeout=timeout, retry_attempts=retry_attempts, retry_sleep_seconds=retry_sleep_seconds)
        ...
        if not batch:
            break
        if total_count is not None and len(all_projects) >= total_count:
            break
    ...
    payload: dict[str, Any] = {
        "source": {
            "page_url": PAGE_URL,
            "api_url": BASE_URL,
            "project_identifier_mapping": {"project_public_id": "GS + sustaincert_id", "project_internal_id": "id"},
            "extraction_method": "gold_standard_public_api",
        },
...
def main() -> int:
    ...
    print("Iniciando extracao da lista de projetos da Gold Standard")
