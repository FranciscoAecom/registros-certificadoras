...
PAGE_URL = "https://registry.isometric.com/project"
QUERY_NAME = "ProjectDetails"
...
query ProjectDetails($projectId: ID!) {
  project(id: $projectId) {
    ... on Project {
      ...
      creditingPeriodEnd
      validatedAt
      agreedExclusiveRegistration
      agreedNotificationOfChanges
      hideCcpAccreditation
      isDemo
      riskOfReversal
      riskOfReversalPercentage
      country { isoAlpha3Code name }
      supplier { id organisation { id name imageUrl } }
      process { key displayName pathway { shortName name description type } }
      creditBalance {
        total { credits creditsKg }
        active { credits creditsKg }
        retired { credits creditsKg }
        hasBufferPool
      }
      protocol { id name slug summary updatedAt riskOfReversal }
      latestStatusUpdate { id createdAt summary }
      projectDocuments { totalCount nodes { id displayName url createdAt submissionDate } }
    }
  }
}
""".strip()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Baixa os detalhes brutos dos projetos da Isometric.")
...
def build_paths(snapshot_date: str) -> tuple[Path, Path, Path]:
    ...
    list_path = root / "data" / "project_standards" / "01_bronze" / "isometric" / snapshot_date / "list" / "projects.json"
    projects_dir = root / "data" / "project_standards" / "01_bronze" / "isometric" / snapshot_date / "projects"
...
def write_failure_log(errors_path: Path, snapshot_date: str, failures: list[dict[str, Any]]) -> None:
    ...
    payload = {
        "source": {"certificadora": "isometric", "snapshot_date": snapshot_date, "script": "extract_project_details.py"},
...
def fetch_project_details(project_id: str, timeout: float, retry_attempts: int, retry_sleep_seconds: float) -> dict[str, Any]:
    payload = {"query": GRAPHQL_QUERY, "variables": {"projectId": project_id}, "operationName": QUERY_NAME}
    attempts_total = retry_attempts + 1
    attempt = 1
    while True:
        req = request.Request(url=API_URL, method="POST", headers=build_headers(project_id), data=json.dumps(payload).encode("utf-8"))
        try:
        ...
        if errors_payload:
            raise RuntimeError(f"Resposta GraphQL com erro para o projeto {project_id}: {json.dumps(errors_payload, ensure_ascii=False)}")
        project_payload = response_payload.get("data", {}).get("project")
        if not isinstance(project_payload, dict):
            raise RuntimeError(f"Resposta inesperada para o projeto {project_id}: campo 'project' ausente.")
...
def main() -> int:
    ...
    print("Iniciando extracao de detalhes de projetos da Isometric")
    ...
    print(f"endpoint de detalhe: {API_URL}")
    ...
    for index, list_project in enumerate(projects, start=1):
        ...
        if not args.overwrite_existing and existing_output_is_valid(output_path, project_id):
            ...
            continue
        detail_url = f"{PAGE_URL}/{project_id}"
        print(f"inicio download do projeto {project_id} ({index}/{total_to_process})")
        try:
            detail_payload = fetch_project_details(project_id=project_id, timeout=args.timeout, retry_attempts=args.retry_attempts, retry_sleep_seconds=args.retry_sleep_seconds)
            payload = {
            ...
            append_failure_entry(errors_path=errors_path, snapshot_date=snapshot_date, entry={
